...
public class CompressorFactory {
    ...
    private static final LZFCompressor LZF = new LZFCompressor();  //初始化lzf压缩
    ...
    static {
        ...
        for (Compressor compressor : compressors) {
            compressorsByTypeX.put(compressor.type(), compressor);  //每种类型的压缩 只有一个？
        }
    ...
    /*
     "compress.default.type", "lzf"   只有lzf压缩，其他会报错
     */
    public static synchronized void configure(Settings settings) {
        for (Compressor compressor : compressors) {
            compressor.configure(settings);  // 配置文件 载入到 compressor中
        }
