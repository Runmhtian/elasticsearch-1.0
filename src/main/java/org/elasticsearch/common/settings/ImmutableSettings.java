...
public class ImmutableSettings implements Settings {
     ...
     * construct it.
     *
     * 通过静态内部类来实现  使用Builder来操作一个map，一旦调用builder中的bulid方法生成ImmutableSettings对象后，其中的setting是不可变的
     * 在java客户端可以通过bulider来设置参数
     *
     * 服务端调用时候直接调用build，得到了个EMPTY_SETTINGS
     */
    public static class Builder implements Settings.Builder {

        //new了一个Settings的子类对象，new ImmutableSettings(Collections.unmodifiableMap(map), classLoader);
        //classLoader 默认为null  在ImmutableSettings中使用了默认的classLoader
