...
//        Loggers.getLogger(Bootstrap.class, tuple.v1().get("name")).info("heap_size {}/{}", JvmStats.jvmStats().mem().heapCommitted(), JvmInfo.jvmInfo().mem().heapMax());
        //通过bootstrap.mlockall配置  来锁定物理内存，防止swap
...
//        }
    ...
    private static Tuple<Settings, Environment> initialSettings() {
        //EMPTY_SETTINGS 实际上就是初始化ImmutableSettings的一个实例，当做参数传入prepareSettings
    ...
    public static void main(String[] args) {
        ...
        try {
            tuple = initialSettings();  //初始化  设置文件  并生成environment
            setupLogging(tuple);
        } catch (Exception e) {
        ...
        // warn if running using the client VM  java 虚拟机的运行模式 client 和server  ，默认根据电脑配置，一般都是server
