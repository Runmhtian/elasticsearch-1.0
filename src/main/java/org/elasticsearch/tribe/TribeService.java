...
public class TribeService extends AbstractLifecycleComponent<TribeService> {
    ...
    //处理关于  tribe node 的相关配置 tribe node可以连接多个集群，tribe node可以跨越多个集群，
    // 它可以接收每个集群的状态，然后合并成一个全局集群的状态，它可以读写所有节点上的数据   设置 tribe是每个节点是一个 tribe 客户端

    /*
    若是settings含有tribe.name属性，则将其他 tribe.*的配置移除
    否则
        获取 前缀是 tribe 的属性，若是null直接返回
        否则
            则这个节点是tribe节点，往setting中添加tribe node相关配置
     */
