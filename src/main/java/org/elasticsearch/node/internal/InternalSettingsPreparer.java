...
public class InternalSettingsPreparer {
    ...
    public static Tuple<Settings, Environment> prepareSettings(Settings pSettings, boolean loadConfigSettings) {
        /*
        三种配置文件
        1.传进来的pSettings，也即是调用prepareSettings的代码中  builder的setting
        2.System.getProperty  中的setting
        3.resource配置文件中

        这个函数主要逻辑根据三种配置，来生成setting  和environment
        ...
        //若是builder中设置了config.ignore_system_properties=true，useSystemProperties=false
        boolean useSystemProperties = !pSettings.getAsBoolean("config.ignore_system_properties", false);
        // just create enough settings to build the environment
        //创建一个新的settingsBuilder，并导入pSettings的设置
        ...
        if (loadConfigSettings) {
            ...
            if (loadFromEnv) {  //导入 resource 中的配置
                try {
